...
**flowsense requires an L2 bridge interface.** It uses AF_PACKET to passively observe traffic traversing a bridge. On
a regular endpoint (laptop, desktop), AF_PACKET on a network interface only captures incoming frames — outgoing packets
generated by the local kernel are not visible. This means flowsense will not detect DPI interference on traffic
originating from the machine it runs on.

Supported deployment scenarios:

- **Linux L2 bridge** (primary) — two physical interfaces bridged together, flowsense listens on `br-lan`
- **Docker bridge** — containers behind a Docker network, flowsense listens on `br-XXXX` (useful for testing)
- **Network namespaces with veth pairs** — traffic between namespaces through a bridge
- **QEMU/VM with bridged networking** — VM traffic passing through host bridge

Not supported:

- Running on a regular laptop/desktop interface (e.g. `wlp0s20f3`, `eth0`) — will only see incoming packets
- Loopback (`lo`) — works for unit testing only, not representative of real DPI interference
...
- **Does not work on endpoints** — requires L2 bridge, not a regular network interface (see Prerequisites)
